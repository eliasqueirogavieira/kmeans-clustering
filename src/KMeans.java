...
public class KMeans<Point extends DataPoint> {
    ...
    // Find the center of each cluster and move the centroid to that center
    private void generateCentroids() {
        for (Cluster cluster : clusters) {
            ...
            for (int i = 0; i < cluster.points.get(0).numDimensions; i++) {
                ...
                int dimension = i;
                Double dimensionMean = cluster.points.stream().mapToDouble(x -> x.dimensions.get(dimension)).average().getAsDouble();
    ...
    private boolean listsEqual(List<DataPoint> first, List<DataPoint> second) {
        ...
        for (int i = 0; i < first.size(); i++) {
            for (int j = 0; j < first.get(0).numDimensions; j++) {
                if (!first.get(i).dimensions.get(j).equals(second.get(i).dimensions.get(j))) {
                    return false;
                }
            }
        }
    ...
    public static void main(String[] args) {
        ...
        KMeans<DataPoint> kmeansTest = new KMeans<>(2, List.of(point1,
                point2, point3));
